...
router.get("/", async (req, res, next) => {
  ...
  productHelper.getAllProducts().then((products) => {
    res.render("user/home", { products, count, user });
  });
});
...
router.get("/view-products", async (req, res, next) => {
  ...
  productHelper.getAllProducts().then((products) => {
    res.render("user/menu", { products, count, user });
  });
});
...
router.post("/reserve",async(req,res)=>{
   var tables=[];
    userHelpers.bookTable(req.body);

     
  
})
router.get("/mytable",verifylogin,async (req,res)=>{
  let tables=await userHelpers.getMyTable(req.session.user._id);
  res.render("user/myTable",{user:req.session.user,table:tables})
})
