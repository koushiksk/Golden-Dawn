...
const verifyAdmin = (req, res, next) => {
  ...
  if (req.session.user){
    ...
    if (req.session.user.email=="ratha@gmail.com") {
...
router.get("/edit-images/:imageno/:prodID/:image", verifyAdmin, (req, res,next) => {
...
router.get("/allTables",verifyAdmin,async(req,res)=>{
  let allTables=await productHelper.getAllTables();
  res.render("admin/allTables",{
    admin:req.session.user.admin,
    tables:allTables,
  })
})
